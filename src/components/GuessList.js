...
function GuessList(props) {
  ...
  const { loading: guessListLoading, data: guessListData } = useQuery(
    guessListQuery
  );
  const { user } = useAuth();
    ...
    return guessListData.guessedPlayers.map(guessedPlayer => (
      ...
      >
        {Math.floor(Math.random() * 10 + 1) % 2 === 1 ? (
          ...
          <Text {...animatedGuessNameStyle}>{guessedPlayer.name}</Text>
        )}
      </Flex>
    ));
  }
  ...
  function dispalayGuesses() {
    ...
    if (user.role !== 'admin')
      return <FlexItem>{displayGuessesNonAdmin()}</FlexItem>;

    return guessListData.guessedPlayers.map(guessedPlayer => {
      return selectedPlayer === guessedPlayer ? (
        <Box height="7vh" my={[4]} mx={[1]}>
          <GuessForm
            form={form}
            inputs={inputs}
            buttons={buttons}
  ...
  return (
    <BoxAll
      ...
      fontSizeModule={[1]}
    >
      <Text isA="h2" m="auto" textAlign="center" h2FontSize={[1]}>
        Todays Guesses
      </Text>
      <FlexItem>{dispalayGuesses()}</FlexItem>
    </BoxAll>
  );
}

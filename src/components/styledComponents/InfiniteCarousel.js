...
const InfiniteCarousel = ({
	...
	const { arrowColor, stroke, strokeWidth } = arrowStyle;
	...
	function buildCssTransformVal(
		...
		if (allCarouselItemsVisible) {
			...
			if (fromBeginningCarouselToEnd) {
				if (firstSlide) {
					...
					return {
						carouselAnimation: fromEndToFront,
					};
				}
				carouselItemTranslate.animation.in['0%'] = fromLowerToUpper;

				return {
					carouselAnimation: carouselItemTranslate,
				};
			}
			...
			if (traverseCarouselUp) {
				if (lastSlide) {
					return {
						carouselAnimation: fromFrontToEnd,
					};
				}
			...
			if (lastSlide) {
				outAnimationFromEnd = {
					animation: {
						in: {
							'0%': {
								transform: `translateX(${bp * i}px)`,
							},
							'100%': {
								transform: `translateX(${bp * 10}px)`,
							},
						},
						duration_in: carouselSpeed,
						animation_fill_mode: 'forwards',
		...
		if (traverseCarouselUp) {
			let outAnimationFromStart = null;
	...
	function createCarouselTranslatVals() {
		...
		while (lowerLimit < upperLimit) {
			...
			if (children[lowerLimit]) {
				carouselPositions[lowerLimit] = carouselAnimation;
				lowerLimit++;
				i++;
				continue;
			}
			...
			if (lowerLimit >= children.length) {
				carouselPositions[lowerLimit - children.length] = carouselAnimation;
				carouselLengthEnd--;
				lowerLimit++;
				i++;
				continue;
			...
			carouselPositions[children.length + lowerLimit] = carouselAnimation;
			carouselLengthEnd--;
			lowerLimit++;
			i++;
			continue;

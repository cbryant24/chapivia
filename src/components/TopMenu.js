...
function TopMenu(props) {
  ...
  function displayUserOptions() {
    if (user) {
      return (
        <Fragment>
          <P className="logout-player" color="red" cursor="pointer" onClick={signout}>
            Logout
          </P>
    ...
    <Div width={[1]} fontSizeModule={[2, 3]}>
      ...
      >
        ...
        >
          ...
          <P className="trivia-topic">{data ? data.localTrivia.category : ""}</P>
        </FlexDiv>
        <FlexDiv themeStyle={["flexColumnCenter"]} width={[2, 3]}>
          ...
          <P className="hi-score">
            {!playerScores || !playerScores.scores.length
              ? 0 : `${playerScores.scores[0].score}0,000` }
          </P>
        </FlexDiv>
